...
import org.muskan.payment.CreditCardPayment;
import org.muskan.payment.PaymentStrategy;
...
public class RentalServiceDriver {
    public static void main(String[] args) {
        ...
        //Adding observers
        notificationManager.addObserver(new EmailNotificationService());
        notificationManager.addObserver(new SMSNotificationService());
        ...
        // Send reminder for upcoming reservation
        notificationManager.notifyObservers(reservation);



        // Process payment
        PaymentStrategy paymentStrategy  = new CreditCardPayment();
        PaymentStatus paymentStatus = paymentStrategy.processPayment(vehicle.calculateRentalPrice());
        if (paymentStatus == PaymentStatus.PAID) {
            System.out.println("Reservation confirmed!");
        }
